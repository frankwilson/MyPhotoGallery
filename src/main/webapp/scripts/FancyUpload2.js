...
(function() {
    var phrases = {
        ...
        'fileName': '{name}',
        'remove': 'Удалить',
        'removeTitle': 'Щёлкните чтоб удалить.',
        'fileError': 'Ошибка загрузки',
        'validationErrors': {
            'duplicate': 'Следующий файл уже добавлен: <em>{name}</em>, повторное добавление невозможно.',
            'sizeLimitMin': 'Файл <em>{name}</em> (<em>{size}</em>) слишком мелкий, минимальный размер {fileSizeMin}.',
            'sizeLimitMax': 'Файл <em>{name}</em> (<em>{size}</em>) слишком большой, максимальный размер <em>{fileSizeMax}</em>.',
            'fileListMax': 'Файл <em>{name}</em> не может быть добавлен из-за ограничения очереди в <em>{fileListMax} файлов</em>.',
            'fileListSizeMax': 'Файл <em>{name}</em> (<em>{size}</em>) слишком большой, общий размер очереди должен быть не более <em>{fileListSizeMax}</em>.'
        },
        'errors': {
            'httpStatus': 'Сервер вернул HTTP-код <code>#{code}</code>',
            'securityError': 'Ошибка безопасности ({text})',
            'ioError': 'Ошибка при отправке файла или обработки его на сервере ({text})'
        },
